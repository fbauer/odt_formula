...
error = p.a('#') + qname(string.uppercase + string.digits) + (oneof('!?') |
                                                              (p.a('/') +  (oneof(string.uppercase) |
...
rangeaddress = (
    ...
    sheetlocatororempty + p.a('.') + column + row + p.maybe(p.a(':') +  p.a('.') + column +  row ) |
...
parameterlist = p.maybe(
parameter + p.many(separator + emptyorparameter ) |
separator +  emptyorparameter + # /* first param empty */
(separator +  emptyorparameter ))
